...
class AccuPassSite(object):
    ...
    def getSoup(self,url,maxRetryTime=20):
    ...
    def getSiteInfo(self):
        j = 0
        while (self.numOfPages == None or j < self.numOfPages):  # 此迴圈將一次抓取一個頁面上的所有活動。註：一個頁面的資訊將會存成一個soup。
            print("\nthis is loop %d." % j)
            url = 'https://www.accupass.com/search/r/1/0/6/1/1/' + str(j) + '/20120101/20161231'
            self.getSoup(url).getNumOfPages().getCols()  # 1.擷取soup 2.若是第一次跑則判斷總共有幾個soup要擷取 3.從soup中擷取欄位資訊
            j += 1
